...
def h_nonlin(X, fnc, eps=1e-12, name='b'):
	...
	b = tf.Variable('b'+name, shape=[1,1,1, msh[3], 1, msh[5]])
...
def bn(X, train_phase, decay=0.99, name='batchNorm'):
	...
	with tf.name_scope(name) as scope:
		beta = tf.Variable(name+'_beta', dtype=tf.float32, shape=n_out,
									  initializer=tf.constant_initializer(0.0))
		gamma = tf.Variable(name+'_gamma', dtype=tf.float32, shape=n_out,
										initializer=tf.constant_initializer(1.0))
		pop_mean = tf.Variable(name+'_pop_mean', dtype=tf.float32,
											shape=n_out, trainable=False)
		pop_var = tf.Variable(name+'_pop_var', dtype=tf.float32,
										  shape=n_out, trainable=False)
...
def get_weights(filter_shape, W_init=None, std_mult=0.4, name='W'):
	...
	"""
	print(f"get_weights({filter_shape=})")
	if W_init == None:
		stddev = std_mult*np.sqrt(2.0 / np.prod(filter_shape[:3]))
		W_init = tf.random_normal_initializer(stddev=stddev)
	initv = W_init(shape=filter_shape, dtype=tf.float32)
	return tf.Variable(name=name, initial_value=initv, dtype=tf.float32, shape=filter_shape)	#mck
